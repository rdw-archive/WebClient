class Window extends Widget {
	constructor(widgetName, parentFrame, template = WIDGET_TEMPLATE_WINDOW) {
		...
		this.titleBar = new Frame(
			widgetName + "TitleBar",
			window,
			WIDGET_TEMPLATE_WINDOW_TITLEBAR
		);
		this.titleText = this.titleBar.createFontString(
		...
		this.minButton = new Frame(
			...
			this.titleBar,
			WIDGET_TEMPLATE_WINDOW_MINIMIZEBUTTON
		);
		this.maxButton = new Frame(
			widgetName + "MaximizeButton",
			this.titleBar,
			WIDGET_TEMPLATE_WINDOW_MAXIMIZEBUTTON
		);
		this.closeButton = new Frame(
			widgetName + "CloseButton",
			this.titleBar,
			WIDGET_TEMPLATE_WINDOW_CLOSEBUTTON
		);
		const closeIcon = this.closeButton.createFontString(
			widgetName + "CloseButtonIcon",
			"HIGH",
			"BoldFontHuge"
		);
		...
		this.content = new Frame(
			widgetName + "Content",
			window,
			WIDGET_TEMPLATE_WINDOW_CONTENTPANE
		);
		...
		this.statusBar = new Frame(
			widgetName + "StatusBar",
			window,
			WIDGET_TEMPLATE_WINDOW_STATUSBAR
		);
		this.statusText = this.statusBar.createFontString(
			widgetName + "StatusText",
			"HIGH",
			"GameFontSmallest"
		);
		this.statusText.setText(
			format("%s: %s (%s)", L["Debug Info"], this.name, template)
		);
	}
	setTitle(text) {
