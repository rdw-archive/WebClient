...
KeyboardInputFrame.onKeyboardInputReceived = function (
	event,
	keyCode,
	keyString
) {
	KeyboardInputFrame.fontString.setText(
		L["Last Key"] + ": " + keyCode + " (" + keyString + ")"
	);
};

KeyboardInputFrame.onLoad = function () {
	const text = KeyboardInputFrame.createFontString(
		"KeyboardInputText",
		"HIGH",
		"GameFontSmallest"
	);
	...
	C_EventSystem.registerEvent(
		"KEY_UP",
		"KeyboardInputFrame",
		this.onKeyboardInputReceived
	); // tbd use KEY_UP if mode is RESPONSIVE? (setting)
