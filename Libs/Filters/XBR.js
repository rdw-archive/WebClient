...
var xBR = (function () {
	...
	function d(pixelA, pixelB) {
		...
		var u = r * -0.168736 + g * -0.331264 + b * 0.5;
		var v = r * 0.5 + g * -0.418688 + b * -0.081312;
			...
			for (var y = 0; y < srcH; ++y) {
				...
				*/
				...
				if (a1 < b1) {
					...
					var blended_pixel = blend(new_pixel, matrix[10], 0.5);
				...
				if (a2 < b2) {
					...
					var blended_pixel = blend(new_pixel, matrix[10], 0.5);
					sPixelView[y * SCALE * scaledWidth + (x * SCALE + 1)] =
						blended_pixel.value;
				} else {
					sPixelView[y * SCALE * scaledWidth + (x * SCALE + 1)] =
						matrix[10].value;
				...
				if (a4 < b4) {
					...
					var blended_pixel = blend(new_pixel, matrix[10], 0.5);
					sPixelView[(y * SCALE + 1) * scaledWidth + (x * SCALE + 1)] =
						blended_pixel.value;
				} else {
					sPixelView[(y * SCALE + 1) * scaledWidth + (x * SCALE + 1)] =
						matrix[10].value;
