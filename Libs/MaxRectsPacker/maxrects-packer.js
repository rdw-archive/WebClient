...
class MaxRectsBin extends Bin {
	constructor(
		maxWidth = EDGE_MAX_VALUE,
		maxHeight = EDGE_MAX_VALUE,
		padding = 0,
		options = {
			smart: true,
			pot: true,
	...
	findNode(width, height, allowRotation) {
		...
		for (let i in this.freeRects) {
			...
			if (r.width >= width && r.height >= height) {
				areaFit =
					this.options.logic === exports.PACKING_LOGIC.MAX_AREA
