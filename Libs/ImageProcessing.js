...
function Bitmap_CreateTextureAtlas(
	frames,
	addOuterPadding = ADD_OUTER_PADDING,
	clearColor,
	atlasWidth = DEFAULT_SPRITESHEET_WIDTH,
	atlasHeight = DEFAULT_SPRITESHEET_HEIGHT
) {
	...
	function Bitmap_DrawSprite(
		...
		for (let sourcePixelY = 1; sourcePixelY <= frameHeight; sourcePixelY++) {
			// Why start at 1 instead of 0, or vice versa? (due to the height calculation?? it takes y - 1 so it expects the first one to be 1 and not zero)
			for (let sourcePixelX = 0; sourcePixelX < frameWidth; sourcePixelX++) {
				// Changing this seems to cause glitches
		...
		let bottomRightPadding = getImageSlice(
			pixelData,
			frameWidth - BORDER_SIZE_PIXELS / 2,
			frameHeight - BORDER_SIZE_PIXELS / 2,
			BORDER_SIZE_PIXELS / 2,
			BORDER_SIZE_PIXELS / 2
		);
		let bottomLeftPadding = getImageSlice(
			pixelData,
			0,
			frameHeight - BORDER_SIZE_PIXELS / 2,
			BORDER_SIZE_PIXELS / 2,
		...
		putImageSlice(
			spritesheetPixels,
			topLeftPadding,
			offsetX - BORDER_SIZE_PIXELS / 2,
			offsetY - BORDER_SIZE_PIXELS / 2,
			BORDER_SIZE_PIXELS / 2,
			BORDER_SIZE_PIXELS / 2
		);
		putImageSlice(
	...
	if (numSpritesheets > 1) {
		throw new Error(
			"Failed to combine sprites into a single " +
				atlasWidth +
				"x" +
				atlasHeight +
				" texture atlas (limit is too small?)"
		);
